...
impl Read for Entry {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, io::Error> {
        ...
        self.position = self
            ...
            .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "position out of bound"))?;
        Ok(read)
    }
}
        ...
        let offset = match pos {
            io::SeekFrom::Start(offset) => {
                if offset >= self.position {
                    let offset = offset - self.position;
                    offset
                        .try_into()
                        .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?
                } else {
            ...
            io::SeekFrom::Current(offset) => offset,
            io::SeekFrom::End(offset) => {
                self.position = self.file.seek(io::SeekFrom::End(offset))?;
                return Ok(self.position);
            }
        };
        self.position = self.position.checked_add_signed(offset).ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidData, "Unable to calculate new_pos")
        })?;
        ...
        if convert_to_utf8 {
            ...
            if !path_converted.exists() {
                ...
                if let Some((codepage, offset)) = pair {
                    ...
                    let mut reader = UTF8DecReader::for_label(codepage, file).ok_or_else(|| {
                        io::Error::new(io::ErrorKind::InvalidData, "Unable to create UTF8DecReader")
                    })?;

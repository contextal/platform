...
fn process_request(
    ...
    let mut input_file = std::fs::File::open(input_path)?.take(config.max_child_input_size + 1);
    ...
    let output_size = {
        let mut xz = lzma::LzmaStream::new_auto(&mut input_file)?;
        match std::io::copy(&mut xz, &mut outf) {
            Err(e) => {
                match e.kind() {
    ...
    if let Some(serde_json::Value::String(name)) = request.relation_metadata.get("name") {
        let name = if name.ends_with(".lzma") || name.ends_with(".lzip") {
            Some(name[0..(name.len() - 5)].to_string())
        } else if name.ends_with(".xz") || name.ends_with(".lz") {
    ...
    let remaining_input_size = input_file.limit();
